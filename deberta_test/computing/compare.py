...
def get_similarity(func_model, tokenizer, sentence_1, sentence_2, percent=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # compute embeddings
    inputs_1 = tokenizer.encode_plus(sentence_1, return_tensors='pt', padding=True).to(device)
    inputs_2 = tokenizer.encode_plus(sentence_2, return_tensors='pt', padding=True).to(device)
    
    with torch.no_grad():
        outputs_1 = func_model(**inputs_1).last_hidden_state[:, 0, :].cpu()
        outputs_2 = func_model(**inputs_2).last_hidden_state[:, 0, :].cpu()
...
"""
def compare_skill_arr_method_2(model, tokenizer, skill_eval_arr, base_arr, percent=True):
    similarity_score = 0
    ...
    for cv_skill in skill_eval_arr:
        for base_skill in base_arr:
            similarity = np.append(similarity, get_similarity(model, tokenizer, base_skill['skill'], cv_skill['skill'], percent=percent))
...
"""
def compare_skill_arr_method_1(model, tokenizer, skill_eval_arr, base_arr, percent=True):
    similarity = 0
    weight = np.array([])
    weight_base = np.array([])
    for data2 in skill_eval_arr:
        for data1 in base_arr:
            similarity += (get_similarity(model, tokenizer, data1['skill'], data2['skill'], percent=percent) * data2['score'] * data1['score'])
